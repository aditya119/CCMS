...
namespace CCMS.Client
{
    public partial class CaseClient
    {
        ...
        {
            return _http.GetFromJsonAsync<PendingDisposedCountModel>($"{insightsBaseUrl}");
        }

        public Task<IEnumerable<ParameterisedReportModel>> GetParameterisedReportAsync(ReportFilterModel reportFilters)
        {
            return _http.GetFromJsonAsync<IEnumerable<ParameterisedReportModel>>($"{insightsBaseUrl}?{reportFilters.QueryString}");
        }
    }
}
